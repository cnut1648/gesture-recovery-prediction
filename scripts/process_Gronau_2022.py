...
out_dir = Path("/home/jiashu/seq/processed") / "USC+Gronau" / "k0"
...
labels = pd.read_excel("/home/jiashu/seq/raw_data/Clinical Data Gronau 30 cases (updated).xlsx", engine="openpyxl")
for case in seq:
    labels_row = labels.query('Case_ID == @case')
    # has 1 and only 1 row
    assert labels_row.shape[0] == 1
    label = labels_row['ESI_12m'].values[0]
